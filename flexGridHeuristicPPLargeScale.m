...
%Date: April 2020
%Description: Heuristic Strategy based on a pre-partitioning method
%(divide-and conquer) for spectrum Allocation in Flex-grid optical
%communications considering frequency shifting capabilities
%The complexity of this strategy is not linear but it is much less than the
%optimal solution (brute-force).
%The reduction in complexity can be controlled with the increase in the
...
for iterCase = 1:1 %iterations for the case under analysis
    ...
    for iterFs = 0:5 %iterations for Fs %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %--- Number of partitions
        partInfo = 10; %number of partitions to perform the pre-partitioning strategy
        ...
        for i = 1:n
            ...
            %by a larger number
            services(i,5) = 12.5 * randi([1 4],1,1);
        ...
        %--------------------------------------------------------------------------
        %Partitioning phase (Divide and conquer solution)
        %--------------------------------------------------------------------------
        
        infSerPart = servicesFs(:,1); %information of services in the system
        
        
        totalVarServProc = []; %total information of variations of services processed
        totalServProc = []; %total information of services processed
        
        numPart = partInfo; %number of partitions
        stepPart = n/numPart; %step among partitions
        divServ = round(1:stepPart:n);
        lenDivServ = length(divServ);
        
        auxPwRefLevel = 1; %auxiliar variable to compute the optimal spectrum for allocation
        
        %-Sorting according the initial frequency, form the lower frequency
        %up to the higher frequency
        auxFreqServInfo = freqServInfo(:,4);
        [valSortFreqServ, idSortFreqServ] = sort(auxFreqServInfo);
        
        %---
        tempPwRefLevel = PwRefLevel;
        %---
        
        for iterPar = 1:lenDivServ %loop related to the prepartitioning method
            
            if iterPar < lenDivServ
                %prepart = infSerPart(divServ(iterPar):divServ(iterPar+1)-1,:);
                prepart = idSortFreqServ(divServ(iterPar):divServ(iterPar+1)-1,:);
                %disp(prepart);
            else
                %prepart = infSerPart(divServ(iterPar):end,:);
                prepart = idSortFreqServ(divServ(iterPar):end,:);
                %disp(prepart);
            end
            auxPrepart = sort(prepart); %auxiliar variable for computing the combinations of variations
            
            %Row information according partitions
            disp(['Patition: ',num2str(iterPar)]);
            disp(prepart);
            [rPrepart,cPrepart] = size(prepart); %size of partitions
            
            %Channel bandwidth of the processed services
            bkInfoPart = bkInfo(auxPrepart);
            BWFsServPart = sum(bkInfoPart);
            
            %Power allocation for the partitions
%             PwRefLevel = zeros(1,maxBW); %power reference for each partition
%             PwRefLevel(1,auxPwRefLevel: (auxPwRefLevel + BWFsServPart-1)) = 1;
%             auxPwRefLevel = auxPwRefLevel + BWFsServPart; %updated value of auxiliar variable
            
            %----
            auxPwRefLevel = auxPwRefLevel + BWFsServPart; %updated value of auxiliar variable
            PwRefLevel(1,auxPwRefLevel:end) = 0; 
            %----
            
            
            %ID variation of service according to the partititon
            varIdPart = find(ismember(varInfo,prepart));
            idVarPart = 1:length(varIdPart);
            
            %Information services according to corresponding variations
            varServPart = varInfo(varIdPart);
            auxVarSerPart = zeros(1,length(varServPart));
            
            for iterSerVar = 1:rPrepart
                auxVarSerPart(find(varServPart == auxPrepart(iterSerVar))) = iterSerVar;
            end
            
            %Priority of services belonging to the current partition
            priVarServPart = priorVarInfo(varIdPart);
            
            %Frequency shifting of services belonging to the current partition
            mFsVarPart = mFsVarInfo(varIdPart);
            
            %Power demanded by services belonging to the current partition
            powVarSerPart = powVarServ(varIdPart,:);
            
            disp('TABLE III: Valid Variations per Service');
            disp('---------------- Variations per service ---------------------');
            disp('- Numb.-ID Var - Sk- auxSk- lk - mts - Power demanded per time slot)');
            disp([idVarPart', varIdPart', varServPart',auxVarSerPart',priVarServPart', mFsVarPart']);
            ...
            costLk = (priorVarInfo(varIdPart))'; %the corresponds to priority level
            
            %--- Cost related to the time shifting performed: cmts
            costMk = (mFsVarInfo(varIdPart))'; %the cost corresponds to the time shifting value
            ...
            [numRowCombVar,numColCombVar,infCombVar,...
                combVar] = funcCombVarSer(auxVarSerPart);
            
            disp(' ');
            disp('------------------------------------------------------');
            if length(unique(auxVarSerPart)) < n
                disp('There is no valid combination ');
            else
                disp(['Number of combinations of variations of services per partition: ',...
                    num2str(numRowCombVar)]);
            end
            
            disp('TABLE IV: Combinations of variations of services per partition');
            ...
            for i = 1:numRowCombVar % i number of combinations
                ...
                for j = 1:numColCombVar %i number of columns of combinations
                    aux = powVarSerPart(combVar(i,j),:); %current value of a variation
            ...
            disp(' ');
            disp('TABLE V: Power Consumed of Combinations of variations of services');
            ...
            disp(' ');
            disp('TABLE VI: Cost related to priority of Combinations of variations of services');
            disp(' -------------- Cost of priority of combination of variations -------------');
            disp('- NumComb - costLk --------------------------------------------------------');
            disp([infCombVar',costLkCombVar]);
            
            disp(' ');
            disp('TABLE VII: Cost related to time shifting of Combinations of variations of services');
            ...
            disp(' ');
            disp('TABLE VIII: Residual Power of Combinations of Variations of Services');
            ...
            [arCombVar,costARCombVar,powCombVar,comVarServ] = funcARCostCombVarSer(numRowCombVar,...
                resPowCombVar,combVar,rPrepart,maxBW,PwRefLevel,BWFs,priVarServPart,...
                powVarSerPart,consumedPowerComb,bkInfoPart,optionAR);
            
            
            disp(' ');
            disp('TABLE IX: Acceptance Ratio, Cost related to rejection of service, and Real power consumed combinations ');
            ...
            disp(' ');
            disp('TABLE X: Summarized information of Combinations of Variations');
            disp(' --------------------------------- Cost of acceptance ratio -----------------------------------------------------------');
            ...
            %---
            PwRefLevel = resPowBestCombVar; %updated value of the resource available for the next partition
            PwRefLevel(PwRefLevel<0)=0; %to ensure only possitive values
            PwRefLevel(1,auxPwRefLevel:end) = 1; 
            %---
            ...
            %Information of variations of services processed belongig the
            %partition
            infCombVarSerProc = infBestCombVar;
            infCombVarSerProc(infCombVarSerProc == 0) = []; %delete the columns with zeros
            
            %-Information of variations of service
            infVarSerProc = varIdPart(infCombVarSerProc);
            totalVarServProc = [totalVarServProc,infVarSerProc]; %total information of variations of services processed
            
            %-Information of services processed
            infServProc = varServPart(infCombVarSerProc);
            totalServProc = [totalServProc,infServProc]; %total information of services processed
            
        end
        
        
        %-Information of power of processed services
        tempPowServProc = powVarServ(totalVarServProc,:);
        powServProc = sum(tempPowServProc,1);
        
        %Service ID per frequency slot
        infPowServProc = zeros(length(totalServProc),maxBW);
        for i = 1:length(totalServProc)
            infPowServProc(i,:) = tempPowServProc(i,:)*totalServProc(i);
        end
        
        %--- Acceptance ratio
        arBestCombVar = (length(totalServProc)/n)*100;
        ...
        useBWServ = sum(services(totalServProc,5));
        
        %-Percentage of utilization of available spectrum considering only
        %the processed services
        useBWServFreSlots = sum(servicesFs(totalServProc,5));
...
solHeurFlexGridLS = [caseInfo,frecShiftingInfo,acceptanceRatioInfo,useAvaBWInfo,...
    useAvaBWpercentInfo,numFreqShiftinInfo,cumFreqShiftinInfo];
save('solHeurFlexGridLS.mat','solHeurFlexGridLS',...
